...
(define (script-fu-bhax-mandala text text2 font fontsize width height color gradient) ;a függvény paraméterei
(let*                                                                                 ;lokális változók bevezetése a programba
    (
        (image (car (gimp-image-new width height 0)))                                 ; 0 = típus, RGB kép
    ...
    (gimp-image-insert-layer image layer 0 0) ;hozzáadjuk a réteget

    (gimp-context-set-foreground '(0 255 0)) ;háttér színének beállítása
    (gimp-drawable-fill layer FILL-FOREGROUND) ;kanna, hátteret tölti ki a választott színnel
    (gimp-image-undo-disable image) 

    (gimp-context-set-foreground color) ;betűszín megadása color helyén

    (set! textfs (car (gimp-text-layer-new image text font fontsize PIXELS))) ;szöveget kiírja
    ...
    (set! textfs (car (gimp-text-layer-new image text2 font fontsize PIXELS))) ;BHAX szöveg ráírása a mandalára
    (gimp-image-insert-layer image textfs 0 -1)
    (gimp-message (number->string text2-height))
    (gimp-layer-set-offsets textfs (- (/ width 2) (/ text2-width 2)) (- (/ height 2) (/ text2-height 2))) ;középre igazítja a BHAX feliratot
...
;(script-fu-bhax-mandala "Tóth Csenge" "Mandala" "Ruge Boogie" 120 1920 1080 '(255 0 0) "Tropical Colors")

(script-fu-register "script-fu-bhax-mandala"  ;menüregisztráló függvény meghívódik
    "Mandala9"
